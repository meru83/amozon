...
$pieces = $_POST['pieces'];
$price = $_POST['price'];
error_log($product_id $size $color $pieces $price, 3, 'error.log');
// $error_message = "";

// try{
//     //selectで同じ構成があったらinsertしないようにする
//     //errorはresponseで返す
//     $selectSql = "SELECT product_id FROM color_size WHERE size = ? && color_code = ?";
//     $selectStmt = $conn->prepare($selectSql);
//     $selectStmt->bind_param("ss", $size, $color);
//     $selectStmt->execute();
